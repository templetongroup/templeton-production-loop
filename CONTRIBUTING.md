...
Model children receive only staged trees without `.git` or GitHub credentials. They may propose bounded staged changes or report findings, but they do not run Git/GitHub effects or trusted verifiers. The deterministic host broker validates and applies accepted changes, runs pinned container verifiers, and may push branches or open pull requests. Neither children nor the broker may merge, enable auto-merge, deploy, publish, purchase, or mutate production without explicit human authorization.
