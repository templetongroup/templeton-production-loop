...
description: Claim one human-approved GitHub issue, implement its exact contract in an isolated git worktree, verify it, and open a PR. One invocation performs one bounded pass.
version: 0.2.0
license: MIT
---

# Templeton Loop — Build for OpenClaw

One pass repairs one PR carrying `loop:changes-requested`, or builds one approved issue. Never merge or deploy.

## 0. Preflight

- Confirm the target repository path, reachable `origin`, and GitHub identity.
- Detect the default branch with `gh repo view --json defaultBranchRef --jq .defaultBranchRef.name`; never hardcode `main`.
- Read repository instructions and commands before editing.
- Never edit a dirty checkout, stash/reset unrelated work, or use the OpenClaw agent workspace itself as disposable state.
- Use a dedicated git worktree for the selected issue or PR. If the runtime already supplied an isolated worktree, keep it and do not nest another.

## 1. Repair queue first

List open PRs labeled `loop:changes-requested`. Skip `loop:needs-human-review` and `loop:stuck`. Choose the least recently updated. Read the linked GitHub issue and latest `Templeton Loop review of COMMIT_SHA` verdict.

Use a dedicated repair worktree for that PR's head branch. Confirm the branch is writable before changing it. Fix only must-fix findings, rerun relevant checks, push to the existing PR branch, remove `loop:changes-requested`, and comment with evidence. End the pass.
...
## 3. Create isolated execution state

Fetch the latest remote default branch. Create or resume an issue-specific branch and dedicated worktree outside the primary checkout, using a predictable name such as `loop/issue-N-short-slug`. Never overwrite an existing worktree or branch with unrelated changes. Record the worktree path in the final result.
...
- Do not claim success from invented output; report real command results.

## 6. Ship a proposal

Commit and push the worktree branch. Open a PR whose body includes:
...
- risk and rollback;
- deployment status: `Not deployed` unless the human owner explicitly requested deployment.
