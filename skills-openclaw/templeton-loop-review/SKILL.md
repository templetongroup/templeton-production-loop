...
# Templeton Loop — Review for OpenClaw

One invocation reviews one PR. The reviewer must use a fresh OpenClaw session key and must not share the builder's context.
...
- Use a read-only checkout or disposable review worktree if local execution is needed. Never push from it.
