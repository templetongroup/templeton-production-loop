...
- The comment's second line is a broker-controlled `Review-State` record written before model-controlled text. Queue scans trust review evidence only from the authenticated broker actor and settle a complete review only when its exact expected terminal label is present without an incompatible automated label. Partial, skipped, missing, malformed, mismatched, or untrusted state always requeues.
